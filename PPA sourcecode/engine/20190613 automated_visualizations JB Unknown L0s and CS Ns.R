...
for (agg in aggregation) {
    ...
    cat("Processing Aggregation Level", agg, "for", mydata, "/n")
    ...
    diagnostic_data <- data_agg %>%
        select(Sector_Level:Level_Level, matches("Diagnostic\\.[0-9]\\.Avail")) %>%
        gather(key = Diagnostic, value = Availability, 4:ncol(.)) %>%
        mutate(Diagnostic = gsub("Diagnostic\\.[0-9]\\.Availability_([a-zA-Z]+)(\\.Availability)?", "\\1", Diagnostic)) %>%
    ...
    ##
    trt_data <- data_agg %>%
        select(Sector_Level:Level_Level, matches("Treatment\\.[0-9]\\.Avail")) %>%
        gather(key = Treatment, value = Availability, 4:ncol(.)) %>%
        mutate(Treatment = gsub("Treatment\\.[0-9]\\.Availability_([a-zA-Z]+)(\\.Availability)?", "\\1", Treatment)) %>%
        arrange(Sector_Level) %>%
